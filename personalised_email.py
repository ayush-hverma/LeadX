...
def generate_email_for_single_lead(lead_details: dict, product_details: str) -> dict:
    ...
    try:
        # Extract product name from product_details
        product_name = None
        ...
        if not product_name:
            # If no exact match found, try to find the first product mentioned
...
At CoinDCX, we're leading India towards the decentralized future of Web3 with an unwavering commitment to safety, simplicity, and education.""",
    ...
    st.markdown(f"[**Sign in with Outlook**]({outlook_auth_url})", unsafe_allow_html=True)
